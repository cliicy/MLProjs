...
def _run_project(project, entry_point, work_dir, parameters, use_conda, storage_dir, experiment_id):
    ...
    try:
        #---->added by cliicy.luo to make it run on windows 06/2018
        if os.environ.get("SHELL",""):
            eprint("==== run cmd from shell ====")
            process.exec_cmd([os.environ.get("SHELL", "bash"), "-c", command], cwd=work_dir,
                             stream_output=True, env=env_map)
        else:
            eprint("==== run cmd from windows ====")
            cmd = '%s %s %s %s' %("pushd",work_dir,"&",run_project_command)
            eprint("==== windows run from work_dir " + cmd + " ====")
            os.system(cmd)
        # <----added by cliicy.luo to make it run on windows 06/2018
        tracking.end_run()
